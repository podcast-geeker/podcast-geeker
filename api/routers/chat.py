...
from podcast_geeker.domain.notebook import ChatSession, Note, Notebook, Source
from podcast_geeker.exceptions import (
    NotFoundError,
)
from podcast_geeker.graphs.chat import graph as chat_graph
...
async def execute_chat(request: ExecuteChatRequest):
    ...
    try:
        ...
        # Execute chat graph in a worker thread to avoid blocking the event loop
...
async def build_context(request: BuildContextRequest):
    ...
    try:
        ...
        async def _build_source_context(
            ...
            try:
                if "insights" in status:
                    return await source.get_context(context_size="short")
        ...
        if request.context_config:
            source_tasks = [
                _build_source_context(source_id, status)
                for source_id, status in request.context_config.get("sources", {}).items()
            ]
            note_tasks = [
                _build_note_context(note_id, status)
            ...
            for source_context in source_results:
                if isinstance(source_context, Exception) or source_context is None:
                    continue
                context_data["sources"].append(source_context)
                total_content += str(source_context)

            for note_context in note_results:
                if isinstance(note_context, Exception) or note_context is None:
                    continue
            ...
            source_results = await asyncio.gather(
                *(source.get_context(context_size="short") for source in sources),
                return_exceptions=True,
            )
            for source_context in source_results:
                if isinstance(source_context, Exception):
                    logger.warning(f"Error processing source context: {source_context}")
                    continue
                context_data["sources"].append(source_context)
                total_content += str(source_context)
            ...
            note_results = await asyncio.gather(
                *(asyncio.to_thread(note.get_context, "short") for note in notes),
                return_exceptions=True,
            )
            for note_context in note_results:
                if isinstance(note_context, Exception):
                    logger.warning(f"Error processing note context: {note_context}")
                    continue
                context_data["notes"].append(note_context)
                total_content += str(note_context)
        ...
        try:
            from podcast_geeker.utils import token_count
