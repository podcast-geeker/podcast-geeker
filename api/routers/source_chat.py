...
MISSING_MODEL_NAME_PATTERN = re.compile(r"model '([^']+)' not found", re.IGNORECASE)


async def _invoke_source_chat_graph_with_timeout(
    state_values: dict, session_id: str, model_id: Optional[str]
) -> dict:
    def _invoke_blocking() -> dict:
        ...
        try:
            ...
            try:
                return future.result(timeout=SOURCE_CHAT_INVOKE_TIMEOUT_SECONDS)
            except concurrent.futures.TimeoutError as timeout_error:
                future.cancel()
                raise TimeoutError(
                    f"invoke timed out after {SOURCE_CHAT_INVOKE_TIMEOUT_SECONDS}s"
                ) from timeout_error
        finally:
            executor.shutdown(wait=False, cancel_futures=True)

    try:
        result = await asyncio.to_thread(_invoke_blocking)
        return result
    except Exception:
        raise


async def _resolve_ollama_fallback_model_id(
    missing_model_name: Optional[str],
) -> Optional[str]:
    """
    Resolve a working Ollama language model from registered models and live Ollama tags.
    Prefer smaller models first for better latency.
    """
    try:
        language_models = await Model.get_models_by_type("language")
        ollama_models = [m for m in language_models if m.provider == "ollama"]
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            tags_response = await client.get(f"{base_url.rstrip('/')}/api/tags")
        ...
        candidates = [
            model
            for model in ollama_models
            if model.name in available_names and model.name != missing_model_name
        ]
        if not candidates:
            return None

        def candidate_rank(model: Model) -> tuple[int, str]:
        ...
        logger.warning(
            f"source_chat fallback model resolution failed: {fallback_error}"
        )
        return None
...
async def create_source_chat_session(
    ...
    source_id: str = ApiPath(..., description="Source ID"),
):
...
async def get_source_chat_sessions(source_id: str = ApiPath(..., description="Source ID")):
...
async def get_source_chat_session(
    source_id: str = ApiPath(..., description="Source ID"),
    session_id: str = ApiPath(..., description="Session ID"),
):
...
async def stream_source_chat_response(
    ...
    try:
        ...
        # Execute source chat graph with timeout protection
        try:
            result = await _invoke_source_chat_graph_with_timeout(
                state_values=state_values,
                session_id=session_id,
                model_id=model_override,
            )
        except Exception as invoke_error:
            error_text = str(invoke_error)
            if isinstance(invoke_error, (asyncio.TimeoutError, TimeoutError)):
                raise RuntimeError(
                    f"Source chat model invocation timed out after {SOURCE_CHAT_INVOKE_TIMEOUT_SECONDS}s. "
                    "Please select another model or ensure the selected model is running."
                ) from invoke_error
            if "not found" in error_text.lower():
                missing_model_name: Optional[str] = None
                missing_match = MISSING_MODEL_NAME_PATTERN.search(error_text)
                if missing_match:
                    missing_model_name = missing_match.group(1)

                fallback_model_id = await _resolve_ollama_fallback_model_id(
                    missing_model_name=missing_model_name
                )
                if fallback_model_id and fallback_model_id != model_override:
                    state_values["model_override"] = fallback_model_id
                    logger.info(
                        f"source_chat retry with ollama fallback session={session_id} "
                        f"missing_model={missing_model_name} fallback_model_id={fallback_model_id}"
                    )
                    result = await _invoke_source_chat_graph_with_timeout(
                        state_values=state_values,
                        session_id=session_id,
