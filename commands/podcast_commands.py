...
async def generate_podcast_command(
    ...
    try:
        ...
        # 3. Configure podcast-creator with only the selected profiles
        episode_profile_dict = episode_profile.model_dump()
        episode_profile_dict["outline_provider"] = _normalize_provider_for_podcast_creator(
            episode_profile_dict.get("outline_provider")
        )
        episode_profile_dict["transcript_provider"] = _normalize_provider_for_podcast_creator(
            episode_profile_dict.get("transcript_provider")
        )
        episode_profiles_dict = {episode_profile_dict["name"]: episode_profile_dict}

        speaker_profile_dict = speaker_profile.model_dump()
        speaker_profile_dict["tts_provider"] = _normalize_provider_for_podcast_creator(
            speaker_profile_dict.get("tts_provider")
        )
        normalized_speakers = []
        for speaker in speaker_profile_dict.get("speakers", []):
            ...
            normalized_speaker["voice_id"] = _normalize_voice_id_for_provider_model(
                ...
                speaker_profile_dict.get("tts_provider"),
                speaker_profile_dict.get("tts_model"),
            )
            normalized_speakers.append(normalized_speaker)
        speaker_profile_dict["speakers"] = normalized_speakers
        speaker_profiles_dict = {speaker_profile_dict["name"]: speaker_profile_dict}
        ...
        async def _provision_provider(prov: str) -> None:
        ...
        if providers:
            await asyncio.gather(*(_provision_provider(prov) for prov in providers))
