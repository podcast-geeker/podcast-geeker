...
- Singleton configuration record (record_id: `podcast_geeker:default_models`)
...
- `podcast_geeker.database.repository`: repo_query, ensure_record_id
- `podcast_geeker.domain.base`: ObjectModel, RecordModel base classes
- `podcast_geeker.domain.credential`: Credential for database-stored API keys
- `podcast_geeker.utils`: token_count() for context size detection
...
from podcast_geeker.ai.provision import provision_langchain_model
