...
- **PODCAST_GEEKER_CHUNK_SIZE**: Maximum chunk size in characters (default: 1200)
...
- **PODCAST_GEEKER_CHUNK_OVERLAP**: Overlap between chunks in characters (default: 15% of CHUNK_SIZE)
...
```bash
export PODCAST_GEEKER_CHUNK_SIZE=512
export PODCAST_GEEKER_CHUNK_OVERLAP=50
```
...
- **CHUNK_SIZE**: Configurable via `PODCAST_GEEKER_CHUNK_SIZE` env var (default: 1200)
- **CHUNK_OVERLAP**: Configurable via `PODCAST_GEEKER_CHUNK_OVERLAP` env var (default: 15% of CHUNK_SIZE)
...
- `podcast_geeker.domain.notebook`: Source, Note, SourceInsight models; vector_search function
- `podcast_geeker.ai.models`: model_manager for embedding model access
- `podcast_geeker.exceptions`: DatabaseOperationError, NotFoundError
...
3. **Change chunk size**: Set PODCAST_GEEKER_CHUNK_SIZE and PODCAST_GEEKER_CHUNK_OVERLAP environment variables
...
### Chunking
```python
from podcast_geeker.utils.chunking import chunk_text, detect_content_type, ContentType
...
### Context Building
```python
from podcast_geeker.utils.context_builder import ContextBuilder, ContextConfig
...
**Key behavior**:
- Key source: PODCAST_GEEKER_ENCRYPTION_KEY_FILE (Docker secrets) → PODCAST_GEEKER_ENCRYPTION_KEY (env var)
...
- PODCAST_GEEKER_ENCRYPTION_KEY must be set explicitly (no default)
...
**Usage Example**:
```python
from podcast_geeker.utils.encryption import encrypt_value, decrypt_value
...
# PODCAST_GEEKER_ENCRYPTION_KEY=my-secret-passphrase
```
