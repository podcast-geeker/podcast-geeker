...
- **Credential**: Individual credential records for API keys and provider configuration
  ...
  - **Encryption integration**: Uses `encrypt_value()`/`decrypt_value()` from `podcast_geeker.utils.encryption`
    - Keys encrypted with Fernet before database storage
    - Requires `PODCAST_GEEKER_ENCRYPTION_KEY` environment variable (warns if not set)
  - **Key methods**:
...
- `podcast_geeker.database.repository`: CRUD and relationship functions
- `podcast_geeker.ai.models`: ModelManager for embeddings
