...
export function useSourceChat(sourceId: string) {
  ...
  const sendMessage = useCallback(async (message: string, modelOverride?: string) => {
    ...
    // Resolve effective model override: passed value > session stored value
    const sessionModelOverride = currentSession?.model_override ?? sessions.find(s => s.id === sessionId)?.model_override ?? null
    const effectiveModelOverride = modelOverride ?? sessionModelOverride ?? undefined
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (trimmedLine.startsWith('data: ')) {
            let data: { type?: string; content?: string; message?: string; data?: SourceChatContextIndicator }
            try {
              data = JSON.parse(trimmedLine.slice(6))
            } catch {
              console.error('Error parsing SSE data:', trimmedLine)
              continue
            }
            ...
            if (data.type === 'ai_message') {
              ...
              streamErrorMessage = data.message || 'Stream error'
              break
            }
          }
        }
      ...
      if (!streamErrorMessage && sseBuffer.trim().startsWith('data: ')) {
        try {
          const trailingData = JSON.parse(sseBuffer.trim().slice(6)) as { type?: string; message?: string }
          if (trailingData.type === 'error') {
            streamErrorMessage = trailingData.message || 'Stream error'
          }
        } catch {
  ...
  }, [sourceId, currentSessionId, currentSession, sessions, refetchCurrentSession, queryClient, t])
